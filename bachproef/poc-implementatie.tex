...
\begin{itemize}
	\item \texttt{\_build\_mapping.py}: dit script bouwt de mapping tussen de volledige lijst van Power BI-datasets en de platinum-datasets. Dit script genereert twee vercshillende outputs:
	\begin{itemize}
...
Verwijzingen naar specifieke database-omgevingen, zoals \texttt{lz02-d-dbw-ccdata-main.platinum\_dev} voor de ontwikkelomgeving, worden via de \texttt{.env} configuratie dynamisch ingelezen. De scriptlogica voegt deze omgevingsvariabelen vervolgens programmatisch samen met de tabelnaam uit de YAML om de uiteindelijke FQN op te bouwen. Deze resulterende FQN is noodzakelijk om de tabelentiteiten eenduidig te kunnen identificeren en correct te koppelen binnen OpenMetadata. Zo kunnen deze gewijzigd worden buiten de logica zelf. Deze aanpak zorgt ervoor dat de code en de configuratie ongewijzigd kunnen functioneren in zowel de dev, test als productie-omgeving.\\

De YAML-opzet blijft hierdoor bovendien makkelijk uitbreidbaar en aanpasbaar. Nieuwe datasets of bijkomende tabellen kunnen zonder aanpassing van de code worden toegevoegd, terwijl bestaande mappings eenvoudig verfijnd kunnen worden wanneer extra domeinkennis beschikbaar komt of de datamodellen wijzigen.\\
